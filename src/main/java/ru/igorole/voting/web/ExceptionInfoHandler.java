...
import ru.igorole.voting.util.exception.IllegalRequestDataException;
...
//    private static Logger log = LoggerFactory.getLogger(ExceptionInfoHandler.class);
    ...
    public ErrorInfo bindError(HttpServletRequest req, Exception e) {
        return logAndGetErrorInfoBindError(req, e, false, VALIDATION_ERROR);
    }
    ...
    private static ErrorInfo logAndGetErrorInfoBindError(HttpServletRequest req, Exception e, boolean logException, ErrorType errorType) {
        Throwable rootCause = ValidationUtil.getRootCause(e);
        if (logException) {
            log.error(errorType + " at request " + req.getRequestURL(), rootCause);
        } else {
